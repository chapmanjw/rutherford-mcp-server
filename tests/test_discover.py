...
@pytest.mark.parametrize(
    ...
    [
        ...
        "gdb",
        "jq",
        # program RUNNERS: not interpreters, but each executes attacker-chosen work from its own args
        "pip",
        "pip3",
        "pipx",
        "conda",
        ...
        "nohup",
        "timeout",
        "systemd-run",  # the family is the leading alpha run, which stops at the hyphen
        "schtasks",
        "docker.exe",
        "git.cmd",
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "rover",
        "phpstorm-agent",
        # near-misses on the SHORT runner families (go, gh, at, pi, make, helm): the classifier matches
        # the whole leading-letter run, so a longer name that merely starts with one must survive
        "goose",
        "golem",
        "gohan",
        "ghost",
