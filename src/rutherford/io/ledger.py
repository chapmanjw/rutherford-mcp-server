...
seam -- plus Markdown ``artifacts/`` (the answer a human reads, and a diff for a write run). This
writer is the only place that touches the jobs directory; the delegation service hands it a finished
record and the answer text.

Persistence is best-effort by contract: a write failure must never fail the delegation that produced
the answer, so the caller wraps :meth:`RunLedger.write` and degrades to an unpersisted result. The
writer keeps no business logic -- it serializes and writes, nothing more.

KNOWN LIMITATION (machine round-trip). ``state.toon`` is written for a human or an LLM to re-read --
which it is, faithfully. It is NOT yet guaranteed to round-trip back through ``decode``: python-toon
0.1.3 (the latest) cannot decode an inline array whose elements are quoted (an argv element with a
colon, e.g. ``gemma3:12b``, or other special characters gets quoted on encode and the decoder then
reads the row as a single value). The reader-side roadmap items that parse a record back (job
continuation, analyze) must resolve this first -- by upgrading/patching the TOON codec, adding a
tolerant reader, or writing a machine-readable sidecar -- rather than assuming ``decode(state.toon)``.
"""
